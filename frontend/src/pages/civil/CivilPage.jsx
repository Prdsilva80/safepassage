...
export default function CivilPage() {
  ...
  const [aiForm, setAiForm] = useState({ lat: '', lng: '', has_vehicle: false, needs_medical_attention: false, group_size: 1, language: 'en' })
  ...
  const getAI = async () => {
    setAiLoading(true)
    try {
      const res = await aiAPI.assess({ ...aiForm, lat: parseFloat(aiForm.lat), lng: parseFloat(aiForm.lng), group_size: parseInt(aiForm.group_size) })
  ...
  return (
    <div style={S.page}>
      <p style={S.title}>CIVILIAN SAFETY PORTAL</p>
        ...
        {sosSent ? (
          ...
          <>
            ...
            <div style={{ ...S.row, marginTop: 12 }}>
              <div style={S.field}>
                <label style={S.label}>PEOPLE COUNT</label>
              ...
              <div style={S.field}>
                ...
                <input value={sosForm.message} onChange={e => setSosForm(f => ({...f, message: e.target.value}))} placeholder="Describe your situation" />
              </div>
            </div>
            ...
            <div style={{ display: 'flex', gap: 16, margin: '12px 0' }}>
              <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, cursor: 'pointer' }}>
                ...
                <Heart size={13} color="var(--red)" /> INJURED
              </label>
              <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, cursor: 'pointer' }}>
                ...
                <Users size={13} color="var(--yellow)" /> CHILDREN
              </label>
            </div>
      ...
      <div style={S.card}>
        ...
        <div style={{ display: 'flex', gap: 16, margin: '12px 0', flexWrap: 'wrap' }}>
          {[['has_vehicle','HAS VEHICLE'],['needs_medical_attention','NEEDS MEDICAL'],['has_children','HAS CHILDREN']].map(([key, label]) => (
          ...
          <div style={{ marginTop: 20, borderTop: '1px solid var(--border)', paddingTop: 20 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
              <span style={{
                ...
                color: RISK_COLOR[aiResult.risk_level] || 'var(--text)',
              }}>
            ...
            <p style={{ fontFamily: 'var(--mono)', fontSize: 11, color: 'var(--text2)', marginBottom: 8 }}>IMMEDIATE ACTIONS</p>
            <ul style={{ paddingLeft: 16, display: 'flex', flexDirection: 'column', gap: 4 }}>
              {aiResult.immediate_actions?.map((a, i) => (
                <li key={i} style={{ color: 'var(--yellow)', fontSize: 13 }}>{a}</li>
              ))}
            </ul>

            {aiResult.evacuation_plan && (
              <>
                <p style={{ fontFamily: 'var(--mono)', fontSize: 11, color: 'var(--text2)', margin: '16px 0 8px' }}>EVACUATION PLAN</p>
